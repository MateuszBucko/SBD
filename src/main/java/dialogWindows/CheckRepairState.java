...
public class CheckRepairState {
	...
	public CheckRepairState() {
		...
		statesList.add(new JScrollPane());
					...
					for (Complaint c : selectedProduct.getComplaints()) {
						if (c.getDecision() != null) {
							if (c.getDecision().getIfPositive() != '\u0000') {
								...
								case MapConst.ACCEPTED:
									state += showStateLabel.getText() + " Zaakceptowana";
									break;
								case MapConst.DECLINE:
									state += showStateLabel.getText() + " Odrzucona";
									break;
								case MapConst.END:
									state += showStateLabel.getText() + " Zakończona";
									break;
								case MapConst.FOR_RAPORT:
									state += showStateLabel.getText() + " Czeka na raport";
									break;
								case MapConst.IN_SERVICE:
									state += showStateLabel.getText() + " W serwisie";
									break;
								case MapConst.NEW:
									state += showStateLabel.getText() + " Nowa";
									break;
								case MapConst.NEXT_REPAIR:
									state += showStateLabel.getText() + " Czeka na następną naprawę  ";
									break;
								}
